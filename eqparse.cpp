...
string quaternion_julia_set_equation_parser::emit_fragment_shader_code(void)
{
	string code;
	...
	code += "\n";
	code += "    float length = iterate(z);\n";
	code += "    gl_FragData[0].rgb = vec3(length, length, length);\n";
	code += "}\n";
...
string quaternion_julia_set_equation_parser::emit_vertex_interp_fragment_shader_code(void)
{
	string code;

	code += "#version 110\n";
	code += "\n";
	code += "uniform sampler1D input0;\n";
	code += "uniform sampler1D input1;\n";
	code += "uniform float z_w;\n";
	code += "uniform vec4 c;\n";
	code += "uniform int max_iterations;\n";
	code += "uniform float threshold;\n";
	code += "uniform int vertex_refinement_steps;\n";
	...
	code += "int lessthan(vec4 left, vec4 right)\n";
	code += "{\n";
	code += "    if(left.x < right.x)\n";
	code += "        return 1;\n";
	code += "    else if(left.x > right.x)\n";
	code += "        return 0;\n";
	code += "\n";
	code += "    if(left.y < right.y)\n";
	code += "        return 1;\n";
	code += "    else if(left.y > right.y)\n";
	code += "        return 0;\n";
	code += "\n";
	code += "    if(left.z < right.z)\n";
	code += "        return 1;\n";
	code += "    else if(left.z > right.z)\n";
	code += "        return 0;\n";
	code += "\n";
	code += "    return 0;\n";
	code += "}\n";

	code += "vec3 vertex_interp(vec4 v0, vec4 v1)\n";
	code += "{\n";
	...
	code += "    }\n";
	code += "\n";
	code += "    // Start half-way between the vertices.\n";
	code += "    vec3 result = (v0.xyz + v1.xyz)*0.5;\n";
	code += "\n";
	code += "    // Refine the result, if need be.\n";
	code += "    if(0 < vertex_refinement_steps)\n";
	code += "    {\n";
	code += "        vec3 forward, backward;\n";
	code += "\n";
	code += "        // If v0 is outside of the surface and v1 is inside of the surface ...\n";
	code += "        if(v0.w > v1.w)\n";
	code += "        {\n";
	code += "            forward = v0.xyz;\n";
	code += "            backward = v1.xyz;\n";
	code += "        }\n";
	code += "        else\n";
	code += "        {\n";
	code += "            forward = v1.xyz;\n";
	code += "            backward = v0.xyz;\n";
	code += "        }\n";
	code += "\n";
	code += "        for(int i = 0; i < vertex_refinement_steps; i++)\n";
	code += "        {\n";
	code += "            // If point is in the quaternion Julia set, then move forward by 1/2 of a step, else move backward by 1/2 of a step ...\n";
	code += "            if(threshold > iterate(vec4(result.xyz, z_w)))\n";
	code += "            {\n";
	code += "                backward = result;\n";
	code += "                result += (forward - result)*0.5;\n";
	code += "            }\n";
	code += "            else\n";
	code += "            {\n";
	code += "                forward = result;\n";
	code += "                result += (backward - result)*0.5;\n";
	code += "            }\n";
	code += "        }\n";
	code += "    }\n";
	code += "\n";
